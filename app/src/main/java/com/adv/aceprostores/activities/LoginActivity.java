...
public class LoginActivity extends BaseActivity implements LoaderCallbacks<Cursor> {
    ...
    private void attemptLogin() {
        ...
        if (cancel) {
            ...
            showProgress(true);
            /*Snipped from auto create
            //login activity
            mAuthTask = new UserLoginTask(email, password);
            mAuthTask.execute((Void) null);
            */
            WS.getInstance(this).login(email, password, new OnSelect_Listener() {
                @Override
                public void onStart() {
                ...
                public void onComplete(Object object) {
                    ...
                    if(response.isStatus()){
                        ...
                        prefs.edit().putInt("id_user", response.getUser().getId()).apply();
                        prefs.edit().putString("key", response.getUser().getKey()).apply();
                        prefs.edit().putInt("conn_type", 1).apply();
                ...
                public void onError() {
                    showProgress(false);
                    showToast(getString(R.string.error_api_general));
                }
            });
        }
    }
